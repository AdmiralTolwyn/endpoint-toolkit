...
| Script | Purpose |
|--------|---------|
| [Invoke-PreUpgradeCleanup.ps1](Invoke-PreUpgradeCleanup.ps1) | Reclaims disk space prior to a Windows feature update or after a reference image build - purges per-user `%TEMP%`, `%WinDir%\Temp`, `SoftwareDistribution`, runs `cleanmgr /sagerun` (optionally hidden via SYSTEM scheduled task), and `dism /StartComponentCleanup [/ResetBase]` (both DISM and `/ResetBase` independently skippable). |
