<#
.SYNOPSIS
    AVD / Windows 365 image-bake optimization wrapper around the Virtual Desktop
    Optimization Tool (VDOT) configuration JSON files. Selectively disables tasks,
    services, autologgers, and applies registry / LGPO tweaks for VDI workloads.

.DESCRIPTION
    Inspired by and references The Virtual Desktop Team's Windows_VDOT.ps1:
        https://github.com/The-Virtual-Desktop-Team/Virtual-Desktop-Optimization-Tool

    This script downloads (over HTTPS) the upstream VDOT JSON configuration files for
    the running Windows release and applies the categories selected via -Optimizations.
    ...
    Categories:
      ...
      LGPO                  - Apply local group policy registry settings (or import
                              via LGPO.exe if a local LGPO\ folder exists).
      DiskCleanup           - Sweep .tmp/.dmp/.etl/.evtx/.log/thumbcache files,
                              %TEMP%, %WinDir%\Temp, WER queues, Recycle Bin,
                              BranchCache.
...
.NOTES
    ...
    Version: 2.0.0
    Context: Azure Image Builder / Packer customizer. Runs as SYSTEM. Requires
             internet egress to raw.githubusercontent.com.
    Requires: Windows 10/11 / Server, PowerShell 5.1+, admin.

    Behaviour preserved from the legacy version - this revision only modernises
    the script header, adds #Requires + comment-based help, and surfaces a
    non-zero exit code when the script bails early.
...
PROCESS {
    ...
    If ($Optimizations -contains "NetworkOptimizations" -or $Optimizations -contains "All")
    {
        ...
        # AR: this setting seems to break AIB from my experience
