<#
.SYNOPSIS
    Image-bake hardening + WinGet provisioning + bulk WinGet app install for AVD /
    Windows 365 reference VMs.

.DESCRIPTION
    Single-shot AIB customizer that prepares an image for capture:

      1. Verifies execution context (user, admin, paths) - logged for AIB diagnostics.
      2. Hardens the bake by disabling Windows Auto Updates, Store auto-downloads,
         the language-pack cleanup task, and the Pre-staged App Cleanup / WU Scheduled
         Start tasks (any of which can break Sysprep or rev packages mid-bake).
         Enables AppModelUnlock\AllowAllTrustedApps for sideloading.
      3. Detects the current Microsoft.DesktopAppInstaller (WinGet) state via the
         WinRT PackageManager (handles 'stub' detection that DISM cannot see).
      4. Downloads the latest signed WinGet bundle, license, and x64 dependencies
         from the microsoft/winget-cli GitHub releases API.
      5. Provisions WinGet system-wide via Add-AppxProvisionedPackage with
         StubPackageOption=installfull and verifies the result.
      6. Optionally registers WinGet for the current user and pre-seeds the WinGet
         source MSIX from the public CDN to bypass error 0x8a15000f on first use.
      7. Optionally installs each entry in -AppIds via `winget install --scope machine`.
         Supports per-app argument overrides via either the in-script $DefaultAppArgs
         hashtable or "ID;customargs" inline syntax.
      8. Cleans up vendor desktop shortcuts (static + dynamic patterns derived from
         the supplied AppIds, with a safety blocklist for generic tokens).
      9. Removes user-context Microsoft.Winget.Source so Sysprep does not fail.

.PARAMETER SkipApps
    Skip both source bootstrap and the application install loop. WinGet itself is
    still provisioned. Use for "WinGet only" image bakes.

.PARAMETER AppIds
    Zero or more WinGet package identifiers to install with `--scope machine`.
    Each entry is either:
      * "Microsoft.VisualStudioCode"                    - default args (or override
                                                          from $DefaultAppArgs)
      * "Vendor.Product;DISABLE_X=1 OPT2=Y"             - inline custom override

.PARAMETER SkipUserRegistration
...
.NOTES
    ...
    WinGet source bootstrap reference (error 0x8a15000f):
...
.EXAMPLE
    # WinGet only (pre-Sysprep), no apps, no user registration
    .\UpdateWinGet.ps1 -SkipApps -SkipUserRegistration

.EXAMPLE
    # WinGet + a small app set
    .\UpdateWinGet.ps1 -AppIds 'Microsoft.VisualStudioCode',
