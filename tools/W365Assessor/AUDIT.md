...
scope, and cached tenants remain fixed when adding scopes. CA now requires the
explicit IncludeConditionalAccess switch; opt-out makes no request. Unused
Directory.Read.All was removed. SDK metadata is not token/role/visibility proof,
and actual authorization/transport auditing remains open. Tests mock all sign-in
commands and exercise the actual production block, initializer and CA gate.
