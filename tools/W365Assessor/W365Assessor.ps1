...
<#
.SYNOPSIS
    Windows 365 Assessor - CAF/WAF/LZA readiness assessment for Windows 365.
.DESCRIPTION
    ...
    discovery import, and rich HTML report export. Follows CAF for Windows 365,
...
$Global:DebugLogFile     = Join-Path $env:TEMP "W365Assessor_debug.log"
...
# Reads W365Assessor_UI.xaml, strips designer-only attributes, parses with XamlReader,
# and creates the main WPF Window. Fatal errors show a MessageBox and exit.
# ===============================================================================

$XamlPath = Join-Path $Global:Root "W365Assessor_UI.xaml"
if (-not (Test-Path $XamlPath)) {
    [System.Windows.MessageBox]::Show(
        "CRITICAL: 'W365Assessor_UI.xaml' not found in:`n$Global:Root",
        "Windows 365 Assessor", 'OK', 'Error') | Out-Null
    exit 1
}

$XamlContent = Get-Content $XamlPath -Raw -Encoding UTF8
Write-Host "[XAML] Loaded XAML: $([math]::Round($XamlContent.Length / 1024, 1)) KB" -ForegroundColor DarkGray
$XamlContent = $XamlContent -replace 'Title="Windows 365 Assessor"', "Title=`"$Global:AppTitle`""
...
$lblStatCloudPCs     = $Window.FindName("lblStatCloudPCs")
$lblStatProvPolicies = $Window.FindName("lblStatProvPolicies")
$lblStatUserSettings = $Window.FindName("lblStatUserSettings")
$lblStatANCs         = $Window.FindName("lblStatANCs")
$lblStatImages       = $Window.FindName("lblStatImages")
...
.DESCRIPTION
    ...
    (2) rotating disk file at %TEMP%\W365Assessor_debug.log (2 MB rotation), (3) in-memory
...
.PARAMETER Title
    Dialog header text. Defaults to 'Windows 365 Assessor'.
.PARAMETER Message
...
function Show-ThemedDialog {
    param(
        [string]$Title    = 'Windows 365 Assessor',
        [string]$Message  = '',
...
foreach ($Def in $Global:CheckDefinitions) {
    [void]$Global:Assessment.Checks.Add([PSCustomObject]@{
        ...
        Origin               = $Def.origin         # W365, WAF, CAF, LZA, SEC
        ...
        BestPractice         = $Def.bestPractice         # MS-documented best practice / evidence statement
        Impact               = $Def.impact               # Business/security/cost impact narrative
        Remediation          = $Def.remediation          # Actionable remediation steps
        AppliesTo            = @($Def.appliesTo)         # SKUs / editions the check applies to
...
# Six maturity dimensions tailored to Windows 365 — every catalog category is covered.
...
$Global:MaturityDimensions = [ordered]@{
    IdentitySecurity = @{ Prefixes = @('W365-IAM-','W365-SEC-');                Label = 'Identity & Security' }
    ProvLifecycle    = @{ Prefixes = @('W365-PROV-','W365-INV-','W365-CPC-');  Label = 'Provisioning & Lifecycle' }
    Networking       = @{ Prefixes = @('W365-NET-');                            Label = 'Networking (ANC)' }
    Resilience       = @{ Prefixes = @('W365-USER-');                           Label = 'Resilience & User Settings' }
    OpsGovernance    = @{ Prefixes = @('W365-GOV-','W365-MON-','W365-COST-');   Label = 'Operations & Governance' }
    Experience       = @{ Prefixes = @('W365-IMG-','W365-UX-','W365-LZ-');      Label = 'Experience & Delivery' }
}
...
<#
.SYNOPSIS
    Imports automated check results from an Invoke-W365Discovery JSON file into the assessment.
.DESCRIPTION
...
function Import-DiscoveryJson {
    ...
    try {
        ...
        $lblStatCloudPCs.Text     = "$($Json.Inventory.CloudPCs.Count)"
        $lblStatProvPolicies.Text = "$($Json.Inventory.ProvisioningPolicies.Count)"
        $lblStatUserSettings.Text = "$($Json.Inventory.UserSettings.Count)"
        $lblStatANCs.Text         = "$($Json.Inventory.AzureNetworkConnections.Count)"
        $lblStatImages.Text       = "$($Json.Inventory.DeviceImages.Count)"
        ...
        foreach ($DiscCheck in $Json.CheckResults) {
            ...
            if ($Match) {
                ...
                # Extract object name from discovery ID (strip prefix up to first W365- or use Evidence)
                $ObjName = if ($DiscCheck.Evidence -and $DiscCheck.Evidence.Name) {
                    $DiscCheck.Evidence.Name
                } elseif ($DiscCheck.Id -match '-W365-(.+)$') {
                    $Matches[1]
...
function Sync-CheckDefinitions {
    ...
    foreach ($Check in $Global:Assessment.Checks) {
        ...
        # Map enrichment fields: checks.json camelCase -> assessment PascalCase property
        $FieldMap = @{
            'impact'               = 'Impact'
        ...
        foreach ($SrcField in $FieldMap.Keys) {
            ...
            if ($DstField -in @('AppliesTo','AdditionalReferences')) { $Val = @($Val) }
            if (-not ($Check.PSObject.Properties.Name -contains $DstField)) {
                $Check | Add-Member -NotePropertyName $DstField -NotePropertyValue $Val -Force
            } else {
                # Always refresh from canonical catalog (these are authoritative, not user-edited)
                $Check.$DstField = $Val
            }
        }

        # Update authoritative fields from canonical catalog
        $Check.Description = $Def.description
        $Check.Severity    = $Def.severity
        $Check.Weight      = [int]$Def.weight
        if ($Def.reference) { $Check.Reference = $Def.reference }
        if ($Def.origin)    { $Check.Origin    = $Def.origin }
    }
}
...
function Update-Dashboard {
    ...
    if ($Global:Assessment.Discovery) {
        ...
        $Items = @(
            @{ Label = 'Cloud PCs';            Count = (@($D.Inventory.CloudPCs).Count);              Icon = [char]0xE770 }
            @{ Label = 'Provisioning Policies'; Count = (@($D.Inventory.ProvisioningPolicies).Count); Icon = [char]0xE74C }
            @{ Label = 'User Settings';        Count = (@($D.Inventory.UserSettings).Count);          Icon = [char]0xE713 }
            @{ Label = 'Network Connections';  Count = (@($D.Inventory.AzureNetworkConnections).Count); Icon = [char]0xE968 }
            @{ Label = 'Device Images';        Count = (@($D.Inventory.DeviceImages).Count);          Icon = [char]0xE91B }
            @{ Label = 'Gallery Images';       Count = (@($D.Inventory.GalleryImages).Count);         Icon = [char]0xEB9F }
            @{ Label = 'Audit Events';         Count = (@($D.Inventory.AuditEvents).Count);           Icon = [char]0xE9F9 }
        )
        ...
        $Msg.Text = "No discovery data loaded. Use 'Load Assessment' to import a discovery JSON, or run Invoke-W365Discovery.ps1."
            ...
            if ($Check.Origin) {
                foreach ($Tag in ($Check.Origin -split ',')) {
                    ...
                    $Badge.Background = $Global:CachedBC.ConvertFromString($(switch ($Tag.Trim()) {
                        ...
                        'W365' { if ($Global:IsLightMode) { '#E0E0E0' } else { '#1F1F23' } }
                        default { if ($Global:IsLightMode) { '#E0E0E0' } else { '#1F1F23' } }
                    }))
                    $Badge.ToolTip = switch ($Tag.Trim()) {
                        'WAF' { 'Well-Architected Framework for Windows 365' }
                        'CAF' { 'Cloud Adoption Framework for Windows 365' }
                        'LZA' { 'Windows 365 Landing Zone Accelerator' }
                        'SEC' { 'Windows 365 Security Recommendations' }
                        'FSL' { 'FSLogix Documentation' }
                        'W365' { 'Windows 365 Documentation' }
                        default { $Tag.Trim() }
                    }
                    ...
                    $BadgeTB.Foreground = $Global:CachedBC.ConvertFromString($(switch ($Tag.Trim()) {
                        ...
                        'W365' { if ($Global:IsLightMode) { '#555' } else { '#A1A1AA' } }
...
function Update-ReportPreview {
    ...
    # Title
    & $AddText 'WINDOWS 365 ASSESSMENT REPORT SUMMARY' $Blue
    & $AddLine
    & $AddText ('=' * 50) $Dim
    & $AddLine; & $AddLine
...
<title>Windows 365 Assessment Report$(if($Global:Assessment.CustomerName){" — $(& $enc $Global:Assessment.CustomerName)"})</title>
<style>
:root {
...
.origin-badge.w365{background:var(--surface);color:var(--text-dim)}
...
/* Best Practice / Impact / Remediation callouts (per check) */
.bp-box{margin-top:6px;padding:6px 8px;background:rgba(96,205,255,0.06);border-left:3px solid var(--accent);border-radius:var(--radius-xs);font-size:11px;color:var(--text-secondary);line-height:1.5}
.bp-box strong{color:var(--accent-text)}
...
.remediation-box{margin-top:4px;padding:6px 8px;background:var(--card);border-left:3px solid var(--green-text);border-radius:var(--radius-xs);font-size:11px;color:var(--text-secondary);line-height:1.5;white-space:pre-wrap;word-break:break-word}
.remediation-box strong{color:var(--green-text)}
.applies-tag{display:inline-block;margin:0 4px 0 0;padding:1px 6px;border-radius:3px;font-size:9px;font-weight:600;background:var(--surface);color:var(--text-dim);border:1px solid var(--card-border)}

/* Sources appendix */
.sources-section{margin-top:36px;padding-top:24px;border-top:1px solid var(--border)}
.sources-section h2{font-size:18px;font-weight:700;color:var(--text-bright);margin-bottom:6px}
.sources-section .sources-sub{font-size:12px;color:var(--text-dim);margin-bottom:16px}
.sources-table{width:100%;border-collapse:collapse;font-size:11px;table-layout:fixed}
.sources-table thead th{padding:8px 12px;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.6px;color:var(--text-faint);border-bottom:1px solid var(--border);text-align:left;background:var(--surface)}
.sources-table thead th:nth-child(1){width:90px}
.sources-table thead th:nth-child(2){width:30%}
.sources-table thead th:nth-child(3){width:auto}
.sources-table tbody td{padding:7px 12px;border-bottom:1px solid var(--border);vertical-align:top;word-wrap:break-word}
.sources-table tbody td a{color:var(--accent-text);text-decoration:none;word-break:break-all}
.sources-table tbody td a:hover{text-decoration:underline}
.sources-table tbody tr:hover{background:var(--card-hover)}
...
<body>
<div class="toolbar">
  <h1><span class="footer-dot"></span>Windows 365 Assessment Report</h1>
...
<div class="meta-grid">
  ...
  <div class="meta-item">Tool <span>Windows 365 Assessor v$Global:AppVersion</span></div>
</div>
...
<div class="maturity-subtitle">Composite score across 6 Windows 365 dimensions — every catalog category contributes</div>
...
"@)
        # ─── CATEGORY DATA SUMMARY (W365 inventory-driven narratives) ─
        $CatSummary = $null
        ...
        if ($Inv) {
            switch -Wildcard ($Cat) {
                'Inventory*' {
                    ...
                    if ($CpcCount -gt 0 -or $PpCount -gt 0) {
                        $StatusGroups = @($Inv.CloudPCs | Group-Object Status | Sort-Object Count -Descending | ForEach-Object { "$($_.Name) ($($_.Count))" }) -join ', '
                ...
                'Provisioning*' {
                    $PpCount = @($Inv.ProvisioningPolicies).Count
                    if ($PpCount -gt 0) {
                        $Unassigned = @($Inv.ProvisioningPolicies | Where-Object { $_.AssignmentCount -eq 0 }).Count
                        $SsoOn      = @($Inv.ProvisioningPolicies | Where-Object { $_.EnableSingleSignOn }).Count
                        $LocalAdmin = @($Inv.ProvisioningPolicies | Where-Object { $_.LocalAdminEnabled }).Count
                        $Autopatch  = @($Inv.ProvisioningPolicies | Where-Object { $_.AutopatchEnabled }).Count
                        $CatSummary = "<strong>$PpCount provisioning policy/policies</strong> in tenant. <strong>$SsoOn</strong> have SSO enabled, <strong>$Autopatch</strong> have Autopatch."
                ...
                'User Settings*' {
                    $UsCount = @($Inv.UserSettings).Count
                    if ($UsCount -gt 0) {
                        $Unassigned = @($Inv.UserSettings | Where-Object { $_.AssignmentCount -eq 0 }).Count
                        $DrEnabled  = @($Inv.UserSettings | Where-Object {
                            $_.CrossRegionDisasterRecoverySetting.disasterRecoveryType -and
                            $_.CrossRegionDisasterRecoverySetting.disasterRecoveryType -ne 'notConfigured'
                        }).Count
                ...
                'Network*' {
                    $AncCount = @($Inv.AzureNetworkConnections).Count
                    if ($AncCount -gt 0) {
                        $Healthy   = @($Inv.AzureNetworkConnections | Where-Object { $_.HealthCheckStatus -eq 'passed' }).Count
                        $Failed    = @($Inv.AzureNetworkConnections | Where-Object { $_.HealthCheckStatus -eq 'failed' }).Count
                        $Hybrid    = @($Inv.AzureNetworkConnections | Where-Object { $_.AdDomainName }).Count
                        $InUse     = @($Inv.AzureNetworkConnections | Where-Object { $_.InUse }).Count
                        $CatSummary = "<strong>$AncCount Azure Network Connection(s)</strong>. <strong>$Healthy passed</strong>, <strong>$Failed failed</strong> health checks. <strong>$InUse</strong> in active use."
                        if ($Hybrid -gt 0) { $CatSummary += " <strong>$Hybrid</strong> configured for Hybrid Entra Join (have AD domain name)." }
                    } else {
                ...
                'Images*' {
                    ...
                    $GiCount = @($Inv.GalleryImages).Count
                    $CatSummary = "<strong>$DiCount custom image(s)</strong> and <strong>$GiCount gallery image(s)</strong> available."
                    if ($DiCount -gt 0) {
                        $now = Get-Date
                        $Stale = @($Inv.DeviceImages | Where-Object {
                            $_.LastModifiedDateTime -and (($now - [datetime]$_.LastModifiedDateTime).Days -gt 90)
                        }).Count
                ...
                'Identity*' {
                    ...
                    if ($PpCount -gt 0) {
                        $SsoOn = @($Inv.ProvisioningPolicies | Where-Object { $_.EnableSingleSignOn }).Count
                        $HybridJoin = @($Inv.ProvisioningPolicies | Where-Object {
                            $_.DomainJoinConfigurations -and
                            ($_.DomainJoinConfigurations | Where-Object { $_.domainJoinType -eq 'hybridAzureADJoin' })
                        }).Count
                ...
                'Cost*' {
                    $CpcCount = @($Inv.CloudPCs).Count
                    if ($CpcCount -gt 0) {
                        $Failed = @($Inv.CloudPCs | Where-Object { $_.Status -eq 'failed' }).Count
                        $Grace  = @($Inv.CloudPCs | Where-Object { $_.Status -eq 'inGracePeriod' }).Count
                        $parts = @("<strong>$CpcCount Cloud PC(s)</strong> licensed")
                        if ($Failed -gt 0) { $parts += "<strong>$Failed failed</strong> (consuming license)" }
                ...
                'Landing*' {
                    $AncCount = @($Inv.AzureNetworkConnections).Count
                    $Subs = @($Inv.AzureNetworkConnections | ForEach-Object { $_.SubscriptionId } | Sort-Object -Unique).Count
                    if ($AncCount -gt 0) {
                        $CatSummary = "<strong>$AncCount ANC(s)</strong> spanning <strong>$Subs subscription(s)</strong>. Validate they sit in dedicated W365 application landing zones."
                    } else {
...
"@)
        foreach ($Check in $CatChecks) {
            ...
            $DetailsCell = [System.Text.StringBuilder]::new()
            [void]$DetailsCell.Append((& $enc $Check.Details))
            if ($Check.Notes) {
                [void]$DetailsCell.Append("`n<div style='margin-top:6px;padding:4px 8px;background:var(--accent-dim);border-left:2px solid var(--accent);border-radius:var(--radius-xs);font-size:11px;color:var(--accent-text)'><strong>Note:</strong> $(& $enc $Check.Notes)</div>")
            }
            # BestPractice callout — always render when present (evidence rationale)
            if ($Check.BestPractice) {
                [void]$DetailsCell.Append("`n<div class='bp-box'><strong>Best practice:</strong> $(& $enc $Check.BestPractice)</div>")
            }
            # Impact + Remediation only render for failing/warning rows
            if ($Check.Impact -and $Check.Status -in @('Fail','Warning')) {
                [void]$DetailsCell.Append("`n<div class='impact-box'><strong>Impact:</strong> $(& $enc $Check.Impact)</div>")
            }
            if ($Check.Remediation -and $Check.Status -in @('Fail','Warning')) {
                [void]$DetailsCell.Append("`n<div class='remediation-box'><strong>Fix:</strong> $(& $enc $Check.Remediation)</div>")
            }
            # AppliesTo SKU tags — small badges below details
            if ($Check.AppliesTo -and @($Check.AppliesTo).Count -gt 0) {
                $TagHtml = (@($Check.AppliesTo) | ForEach-Object { "<span class='applies-tag'>$(& $enc $_)</span>" }) -join ''
                [void]$DetailsCell.Append("`n<div style='margin-top:6px'>$TagHtml</div>")
            }
            [void]$html.Append("<td>$($DetailsCell.ToString())</td>")
        ...
        # ─── COMPARISON TABLES (W365 inventory per category) ──────────
        if ($Inv) {
            switch -Wildcard ($Cat) {
                'Inventory*' {
                    $CPCs = @($Inv.CloudPCs)
                    if ($CPCs.Count -gt 0) {
                        $SkuGroups = @($CPCs | Group-Object ServicePlanName | Sort-Object Count -Descending)
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Cloud PC SKU Distribution</div>`n")
                        [void]$html.Append("<table class='comparison-table'><thead><tr><th>Service Plan</th><th>Count</th><th>% of Fleet</th></tr></thead><tbody>`n")
                        foreach ($G in $SkuGroups) {
                            $Pct  = [math]::Round($G.Count / $CPCs.Count * 100, 0)
                            $Name = if ($G.Name) { $G.Name } else { '(unknown)' }
                            [void]$html.Append("<tr><td>$(& $enc $Name)</td><td>$($G.Count)</td><td>$Pct%</td></tr>`n")
                        }
                        [void]$html.Append("</tbody></table>`n")
                    }
                }
                'Provisioning*' {
                    ...
                    if ($PPs.Count -gt 0) {
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Provisioning Policy Comparison</div>`n")
                        [void]$html.Append("<table class='comparison-table'><thead><tr><th>Name</th><th>Image</th><th>SSO</th><th>Local Admin</th><th>Autopatch</th><th>Assignments</th></tr></thead><tbody>`n")
                        foreach ($P in $PPs) {
                            $Sso  = if ($P.EnableSingleSignOn)  { "<span style='color:var(--green-text)'>Yes</span>" } else { "<span style='color:var(--orange-text)'>No</span>" }
                            $La   = if ($P.LocalAdminEnabled)    { "<span style='color:var(--red-text)'>Yes</span>"   } else { "<span style='color:var(--green-text)'>No</span>" }
                            $Ap   = if ($P.AutopatchEnabled)     { "<span style='color:var(--green-text)'>Yes</span>" } else { "<span style='color:var(--text-faint)'>No</span>" }
                            $Asn  = if ($P.AssignmentCount -gt 0){ "<span style='color:var(--green-text)'>$($P.AssignmentCount)</span>" } else { "<span style='color:var(--red-text)'>0</span>" }
                            $Img  = if ($P.ImageDisplayName) { $P.ImageDisplayName } else { '(unknown)' }
                            [void]$html.Append("<tr><td>$(& $enc $P.DisplayName)</td><td>$(& $enc $Img)</td><td>$Sso</td><td>$La</td><td>$Ap</td><td>$Asn</td></tr>`n")
                        }
                        [void]$html.Append("</tbody></table>`n")
                    }
                }
                'Network*' {
                    ...
                    if ($ANCs.Count -gt 0) {
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Azure Network Connections</div>`n")
                        [void]$html.Append("<table class='comparison-table'><thead><tr><th>Name</th><th>Connection Type</th><th>Region</th><th>Health</th><th>In Use</th></tr></thead><tbody>`n")
                        foreach ($A in $ANCs) {
                ...
                'Images*' {
                    ...
                    if ($DIs.Count -gt 0) {
                        $now = Get-Date
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Custom Device Images</div>`n")
                        ...
                        foreach ($D in $DIs) {
                            ...
                            [void]$html.Append("<tr><td>$(& $enc $D.DisplayName)</td><td>$(& $enc $D.OperatingSystem)</td><td>$(& $enc $D.OsBuildNumber)</td><td>$(& $enc $D.Status)</td><td><span style='$AgeColor;font-weight:600'>$Age</span></td></tr>`n")
                        }
                        [void]$html.Append("</tbody></table>`n")
                    }
                    $GIs = @($Inv.GalleryImages)
                    if ($GIs.Count -gt 0) {
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Gallery Images Available</div>`n")
                        [void]$html.Append("<table class='comparison-table'><thead><tr><th>Name</th><th>Publisher</th><th>SKU</th><th>End of Support</th></tr></thead><tbody>`n")
                        foreach ($G in $GIs) {
                            [void]$html.Append("<tr><td>$(& $enc $G.DisplayName)</td><td>$(& $enc $G.PublisherName)</td><td>$(& $enc $G.SkuName)</td><td>$(& $enc $G.EndOfSupportDateTime)</td></tr>`n")
                        }
                        [void]$html.Append("</tbody></table>`n")
                    }
                }
                'Cost*' {
                    $CPCs = @($Inv.CloudPCs)
                    $Failed = @($CPCs | Where-Object { $_.Status -in @('failed','inGracePeriod') })
                    if ($Failed.Count -gt 0) {
                        [void]$html.Append("<div style='margin-top:12px;font-size:13px;font-weight:600;color:var(--text-bright)'>Cloud PCs Wasting Licenses</div>`n")
                        [void]$html.Append("<table class='comparison-table'><thead><tr><th>Name</th><th>User</th><th>Status</th><th>Service Plan</th></tr></thead><tbody>`n")
                        foreach ($C in $Failed) {
                            [void]$html.Append("<tr><td>$(& $enc $C.DisplayName)</td><td>$(& $enc $C.UserPrincipalName)</td><td>$(& $enc $C.Status)</td><td>$(& $enc $C.ServicePlanName)</td></tr>`n")
                        }
                        [void]$html.Append("</tbody></table>`n")
                    }
                }
        ...
        # ─── FIELD NOTES (W365 expert knowledge per category) ─────────
        $FieldNote = $null
        switch -Wildcard ($Cat) {
            'Inventory*' {
                $FieldNote = @"
...
"@
            ...
            'Provisioning*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Provisioning Policies</div>
<p>Provisioning policies are the contract between a license, a user, and the resulting Cloud PC. An unassigned policy is dead configuration; a misconfigured one silently propagates to every PC it provisions.</p>
<ul>
<li><strong>Naming convention</strong> — Encode region + persona + join type in the policy name (e.g. <code>W365-WEU-Dev-EJOIN</code>). Use the <code>cloudPcNamingTemplate</code> field with <code>%USERNAME:5%-%RAND:5%</code> for traceable Cloud PC names.</li>
...
"@
            ...
            'User Settings*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — User Settings & Resilience</div>
<p>Unlike AVD, Windows 365 has <strong>no FSLogix</strong> profile container concept — there is no profile mount, no VHD, no storage account to maintain. The resilience layers are entirely policy-driven: <strong>OneDrive KFM</strong> for files, <strong>restore points</strong> for the Cloud PC, and <strong>cross-region DR</strong> for the region.</p>
<ul>
...
"@
            ...
            'Network*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Network (ANC)</div>
<p>The Azure Network Connection (ANC) is the only network primitive a W365 admin owns directly. ANC health checks are the canary that detects every network-side problem (DNS, AD, NSG, IP exhaustion, endpoint reachability) before any user sees it.</p>
<ul>
<li><strong>Run health checks after every change</strong> — They are idempotent and fast. A failed check blocks all new provisioning on that ANC; existing Cloud PCs may still work for a while, then fail on the next domain join refresh or DNS lookup.</li>
<li><strong>FQDN allowlist</strong> — Use Azure Firewall's <em>Windows365</em> and <em>WindowsVirtualDesktop</em> FQDN tags rather than maintaining individual URL lists. SSL inspection on these endpoints will break things — exclude them from break-and-inspect.</li>
<li><strong>Port 3389</strong> — Closed by default on new Cloud PCs and should stay closed. RDP traffic flows through the AVD reverse-connect broker, not direct 3389. Any inbound 3389 allow rule from the internet is an immediate finding.</li>
<li><strong>RDP Shortpath</strong> — UDP 3390 (managed networks) or STUN/TURN UDP traversal (public). Cuts perceived latency materially. Check it's enabled via Intune RDP properties, not just the firewall rule.</li>
<li><strong>Microsoft-hosted networking</strong> — If the persona doesn't need on-prem connectivity or Hybrid Join, drop the ANC entirely and use Microsoft-hosted. Less to break, less to maintain.</li>
</ul>
</div>
"@
            }
            'Images*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Images & App Delivery</div>
<p>Image strategy is the single biggest lever on first-login experience. Stale images mean Windows Update churns for 30+ minutes after every new provision; bloated images push initial provisioning past acceptable thresholds.</p>
<ul>
<li><strong>Custom vs gallery</strong> — Use <em>gallery</em> for generic personas where Intune handles all app delivery. Use <em>custom</em> when LOB apps, language packs, or hardening must be pre-baked. Building a custom image just to install Office is a smell — use Intune for that.</li>
<li><strong>Image freshness</strong> — Rebuild monthly aligned with Patch Tuesday. Automate with Azure Image Builder. Anything &gt;90 days old is a finding.</li>
<li><strong>Teams optimization</strong> — New Teams (Teams 2.0) has built-in W365/AVD media optimization — no separate plugin. The endpoint must run a recent Windows App / RD client. Verify in Teams → Settings → About — should say <em>Optimized</em>. Note that Full HD 1080p is not supported for Teams on Cloud PC.</li>
<li><strong>App delivery layering</strong> — Image-baked = large/static; Intune Win32 = dynamic/small; MSIX app attach = shared multi-version (rare in W365). Don't bake everything into the image — that's how golden images become 200 GB.</li>
<li><strong>Language packs</strong> — Top 3-5 languages bake in. The rest deploy via Intune. Always include IMEs for CJK languages.</li>
</ul>
</div>
"@
            }
...
<p>Identity is the perimeter. With no on-prem network boundary protecting the Cloud PC, every control depends on Conditional Access + MFA + compliance — and SSO determines whether users actually use those controls or work around them.</p>
<ul>
<li><strong>SSO</strong> — Enable on every provisioning policy. Requires Entra Join (or Hybrid Join + cloud Kerberos trust). Without SSO, users see a second Windows password prompt and start asking IT to "fix it".</li>
<li><strong>Conditional Access</strong> — Target the <em>Windows 365</em> and <em>Azure Virtual Desktop</em> cloud apps. Require MFA, require compliant connecting device, restrict by location. Phishing-resistant authentication strengths (FIDO2, WHfB, certificate) for privileged users.</li>
<li><strong>WHfB + cloud Kerberos trust</strong> — The path to true passwordless Cloud PC sign-in including SSO to on-prem resources. Configure once, deploy via Intune.</li>
<li><strong>RBAC</strong> — Use the built-in <em>Windows 365 Administrator</em>, <em>Cloud PC Reader</em>, and <em>Cloud PC Troubleshooter</em> roles. Global Admin should never be the assignment for daily W365 ops. Wrap all admin roles in PIM.</li>
<li><strong>Session/idle timeouts</strong> — Lock screen at 5 min, RDP idle disconnect at 30 min. Cheap controls; stop unattended sessions cold.</li>
</ul>
</div>
"@
            }
            'Security*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Security & Compliance</div>
<p>Microsoft's published guidance for Windows 365 security has a clear hierarchy: <strong>(1) MFA via CA, (2) MDE for EDR, (3) compliance policy + CA, (4) update rings, (5) least-privilege local accounts, (6) Defender + Purview integration</strong>. If the first three aren't in place, the rest don't matter.</p>
<ul>
<li><strong>BitLocker is not supported on Cloud PCs.</strong> Azure managed disks are encrypted at rest with PMK (or CMK if configured). Including BitLocker in compliance policy targeting Cloud PCs causes mass false-non-compliance — exclude it.</li>
<li><strong>MDE</strong> — Onboard via Intune connector. Onboarded Cloud PCs feed Microsoft 365 Secure Score, MDE dashboards, and respond to automated remediation. Enable Tamper Protection.</li>
<li><strong>Local admin</strong> — Off by default in W365 Enterprise. If a workload genuinely needs elevation, deploy Endpoint Privilege Management (EPM) for JIT, audited elevation rather than enabling local admin globally.</li>
<li><strong>Security baseline</strong> — Apply the <em>Windows 365 Cloud PC security baseline</em> from Intune Endpoint Security. Pre-tuned for W365 (handles BitLocker exclusion, port 3389 hardening, firewall rules).</li>
<li><strong>Device redirections</strong> — Clipboard, drives, USB, cameras default to allowed. For high-sensitivity personas, disable each via Intune RDP properties + pair with Purview Endpoint DLP for monitoring.</li>
<li><strong>Watermarking</strong> — Deterrent control against screen photography. Embeds the user UPN into the session display. Configure via the W365 security baseline or RDP properties.</li>
</ul>
</div>
"@
            }
...
"@
            ...
            'Monitor*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Monitoring & Diagnostics</div>
<p>The W365 admin center surfaces only point-in-time data. Without forwarding to Log Analytics / Sentinel, you have no historical trend data and no alerting beyond the built-in admin center notifications.</p>
<ul>
<li><strong>Endpoint Analytics</strong> — Free, but Cloud PCs must send Required (or Optional) Windows diagnostic data. Enables Startup performance, App reliability, Resource performance, and Connection quality reports — the primary observability layer for Cloud PCs.</li>
<li><strong>Connection quality</strong> — RTT &gt;150 ms, packet loss &gt;5%, or UDP unavailable each correlate with user-perceived poor quality. RDP Shortpath dramatically improves the metrics where the network path supports it.</li>
<li><strong>Audit retention</strong> — W365 audit events live in Graph (<code>virtualEndpoint/auditEvents</code>) but with limited retention. Forward to Sentinel for 90+ day retention and alerting on policy deletion, mass reprovision, or DR activation.</li>
<li><strong>Proactive remediations</strong> — Detection scripts run daily; remediation scripts only when detection reports an issue. High ROI for common Cloud PC issues (DNS cache, OneDrive sync errors, stuck pending-restart).</li>
<li><strong>Built-in alerts</strong> — Enable in Intune Tenant Admin → Alerts: ANC health failure, provisioning failure, license near exhaustion. Notify the ops team, not just an inbox no one reads.</li>
</ul>
</div>
"@
            }
            'Governance*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — Governance & Operations</div>
<p>OneDrive Known Folder Move is the load-bearing control for user data on Windows 365. Without it, every reset / resize / reprovision is a data-loss event.</p>
<ul>
<li><strong>OneDrive KFM</strong> — Deploy the <em>Silently move Windows known folders to OneDrive</em> Intune profile. This is the W365 equivalent of FSLogix — the only thing standing between a user and lost work during any Cloud PC lifecycle event.</li>
<li><strong>Break-glass accounts</strong> — Two cloud-only Global Admin accounts excluded from all Conditional Access. Long passwords in a physical safe. Sign-in monitoring + alerts. Test login quarterly.</li>
<li><strong>Change control</strong> — Provisioning policy changes affect every PC the policy provisions next. Test in a staging policy first. Use Intune audit logs as the source of truth.</li>
<li><strong>Group strategy</strong> — Dedicated SGs per persona/region (e.g. <code>SG-W365-WEU-Dev</code>). Prefer dynamic groups bound to Entra attributes for auto-membership. Don't nest more than 2 levels.</li>
<li><strong>DR testing</strong> — Untested DR is faith. Activate cross-region DR for a small group annually. Microsoft's 7-day auto-failback makes test activation safe (no manual cleanup required).</li>
<li><strong>Helpdesk readiness</strong> — Build W365-specific decision trees: <em>"Cannot connect" → check ANC health → check license → check connectivity report</em>. Avoid escalating routine W365 issues to L3.</li>
</ul>
</div>
"@
            }
            'End-User*' {
                $FieldNote = @"
<div class="field-note">
<div class="field-note-title">Field Notes — End-User Experience</div>
<p>Adoption is won or lost in the first 60 seconds. Slow boot, double sign-in prompts, missing language pack — any of these and the user files a ticket and prefers their physical PC.</p>
<ul>
<li><strong>Windows App</strong> — The unified client (replaces classic Remote Desktop client). Available on Windows, macOS, iOS, Android, and web. Deploy via Intune for managed endpoints; for BYOD point users to <code>windows365.microsoft.com</code> or the Microsoft Store.</li>
<li><strong>Windows 365 Switch</strong> — Integrates the Cloud PC into Windows 11 Task View on the local device. Enables Alt+Tab-like switching between local and Cloud PC desktops. Free polish if endpoints are Windows 11.</li>
<li><strong>Boot performance</strong> — Target &lt;60 seconds boot-to-desktop. Use Endpoint Analytics startup performance to find slow devices. Common offenders: too many startup apps, GPO/Intune policy bloat, AV scan-at-boot.</li>
<li><strong>Multi-monitor</strong> — Windows App supports up to 16 monitors but the user must enable multi-monitor mode in client settings. Document the steps.</li>
<li><strong>GPU SKUs</strong> — CAD/3D/video personas need GPU-enabled service plans. CPU-only fallback to software rendering is unusable for these workloads.</li>
</ul>
</div>
"@
            }
...
<p>Windows 365 has a much smaller Azure footprint than AVD — typically just the VNet, subnet, and ANC. But that small footprint must still sit in the right management group with compatible Azure Policies, or provisioning silently fails.</p>
<ul>
<li><strong>Application landing zone</strong> — Place W365 networking in a dedicated subscription under <em>Corp</em> (if Hybrid Join / on-prem connectivity needed) or <em>Online</em> (cloud-only Entra Join). Don't co-tenant with unrelated workloads.</li>
<li><strong>Azure Policy compatibility</strong> — Common policies that break W365: deny VM SKUs (W365 service VMs), require tags on Microsoft-managed resources, network policies blocking required FQDNs. Use policy exemptions, not blanket disables.</li>
<li><strong>Hub-spoke</strong> — Required if Cloud PCs need shared services (firewall, DNS, on-prem gateway). Verify VNet peering is bidirectional, gateway transit enabled, BGP routes propagated.</li>
<li><strong>Microsoft-hosted alternative</strong> — For Entra-only, no on-prem dependencies, no custom NSGs, drop ANC entirely. Eliminates VNet/subnet/peering scope from your landing zone.</li>
</ul>
</div>
"@
            }
    ...
    # ── SOURCES APPENDIX (every distinct reference URL across the catalog) ──
    $AllRefs = [System.Collections.Generic.List[object]]::new()
    foreach ($Check in $Global:Assessment.Checks) {
        if ($Check.Reference) {
            [void]$AllRefs.Add([PSCustomObject]@{ Id = $Check.Id; Origin = $Check.Origin; Url = $Check.Reference })
        }
        if ($Check.AdditionalReferences) {
            foreach ($Url in @($Check.AdditionalReferences)) {
                if ($Url) { [void]$AllRefs.Add([PSCustomObject]@{ Id = $Check.Id; Origin = $Check.Origin; Url = $Url }) }
            }
        }
    }
    $UniqueRefs = $AllRefs | Group-Object Url | ForEach-Object {
        [PSCustomObject]@{
            Url    = $_.Name
            Ids    = ($_.Group.Id | Sort-Object -Unique) -join ', '
            Origin = ($_.Group.Origin | Sort-Object -Unique) -join ', '
        }
    } | Sort-Object Url
...
<div class="sources-section">
  ...
  <div class="sources-sub">Every check in this report cites at least one Microsoft Learn or Azure architecture source. The table below lists all distinct references used.</div>
  <table class="sources-table">
    <thead><tr><th>Used by</th><th>Origin</th><th>URL</th></tr></thead>
    <tbody>
"@)
    foreach ($R in $UniqueRefs) {
        $OriginBadges = if ($R.Origin) {
            ($R.Origin -split ',' | ForEach-Object {
                $t = $_.Trim().ToLower()
                "<span class='origin-badge $t'>$($_.Trim().ToUpper())</span>"
            }) -join ''
        } else { '' }
        [void]$html.Append("<tr><td style='font-family:Consolas,monospace;font-size:10px;color:var(--text-dim)'>$(& $enc $R.Ids)</td><td>$OriginBadges</td><td><a href='$(& $enc $R.Url)' target='_blank' rel='noopener noreferrer'>$(& $enc $R.Url)</a></td></tr>`n")
    }
    [void]$html.Append("</tbody></table></div>`n")

    # Footer
    [void]$html.Append(@"
<div class="footer">
  <div><span class="footer-dot"></span>Generated by Windows 365 Assessor v$Global:AppVersion on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') &middot; Overall: <strong>$(Get-MaturityLevel $Overall)</strong> ($Overall%)</div>
  <div>
    <a href="https://learn.microsoft.com/en-us/windows-365/enterprise/overview" target="_blank">W365 Docs</a> &middot;
    <a href="https://learn.microsoft.com/en-us/windows-365/enterprise/security-guidelines" target="_blank">Security</a> &middot;
    <a href="https://learn.microsoft.com/en-us/windows-365/enterprise/planning-guide" target="_blank">Planning</a> &middot;
    <a href="https://learn.microsoft.com/en-us/windows-365/enterprise/cross-region-disaster-recovery" target="_blank">DR</a> &middot;
    <a href="https://learn.microsoft.com/en-us/windows-365/enterprise/requirements-network" target="_blank">Network</a>
  </div>
</div>
</div>
<script>
...
function Get-ExecutiveSummary {
    ...
    [void]$sb.AppendLine('Windows 365 Assessor Executive Summary')
    [void]$sb.AppendLine('==============================')
    ...
    if ($Global:Assessment.Discovery) {
        ...
        if ($D.Inventory) {
            [void]$sb.AppendLine("  Cloud PCs:             $($D.Inventory.CloudPCs.Count)")
            [void]$sb.AppendLine("  Provisioning Policies: $($D.Inventory.ProvisioningPolicies.Count)")
            [void]$sb.AppendLine("  User Settings:         $($D.Inventory.UserSettings.Count)")
            [void]$sb.AppendLine("  Network Connections:   $($D.Inventory.AzureNetworkConnections.Count)")
            [void]$sb.AppendLine("  Custom Images:         $($D.Inventory.DeviceImages.Count)")
        }
    }
    [void]$sb.AppendLine('')
...
"@)
    ...
    if ($Global:Assessment.Discovery -and $Global:Assessment.Discovery.Inventory) {
        ...
        [void]$h.Append('<p style="Margin:0;font-size:11px;font-weight:bold;color:#0078D4;letter-spacing:1px;padding-bottom:14px;">&#9729; &#160;ENVIRONMENT</p>')
        [void]$h.Append('<table cellpadding="0" cellspacing="0" border="0" width="100%"><tr>')
        $envItems = @(
            @{ N=$Inv.CloudPCs.Count;                L='Cloud PCs';     Ic='&#9632;' }
            @{ N=$Inv.ProvisioningPolicies.Count;    L='Prov Policies'; Ic='&#9654;' }
            @{ N=$Inv.AzureNetworkConnections.Count; L='ANCs';          Ic='&#9830;' }
            @{ N=$Inv.DeviceImages.Count;            L='Custom Images'; Ic='&#9733;' }
        )
        foreach ($ei in $envItems) {
            ...
            [void]$h.Append("<p style=`"Margin:0;font-size:22px;font-weight:bold;color:#242424;`">$($ei.N)</p>")
            [void]$h.Append("<p style=`"Margin:0;font-size:10px;color:#8A8886;font-weight:600;letter-spacing:0.5px;padding-top:2px;`">$($ei.L.ToUpper())</p>")
            [void]$h.Append('</td>')
        }
        [void]$h.Append('</tr></table>')
        if ($Global:Assessment.Discovery.Subscriptions) {
            $subNames = ($Global:Assessment.Discovery.Subscriptions | ForEach-Object { & $hesc $_.Name }) -join ', '
            [void]$h.Append("<p style=`"Margin:0;font-size:12px;color:#8A8886;padding-top:10px;`">&#128275; $($Global:Assessment.Discovery.Subscriptions.Count) subscription(s): $subNames</p>")
        }
        [void]$h.Append('</td></tr>')
        [void]$h.Append($divider)
    }
    ...
    # ══════════════════════════════════════════════════════════════════════
    [void]$h.Append('<tr><td style="padding:20px 40px 16px 40px;">')
    [void]$h.Append('<p style="Margin:0;font-size:11px;font-weight:bold;color:#0078D4;letter-spacing:1px;padding-bottom:14px;">&#9776; &#160;CATEGORY BREAKDOWN</p>')
    ...
    # ══════════════════════════════════════════════════════════════════════
    [void]$h.Append('<tr><td style="padding:20px 40px 16px 40px;">')
    [void]$h.Append('<p style="Margin:0;font-size:11px;font-weight:bold;color:#0078D4;letter-spacing:1px;padding-bottom:14px;">&#9670; &#160;MATURITY DIMENSIONS</p>')
    [void]$h.Append('<table cellpadding="0" cellspacing="0" border="0" width="100%" style="font-size:13px;">')
    $dimIdx = 0
    foreach ($Key in $Global:MaturityDimensions.Keys) {
        $Dim    = $Global:MaturityDimensions[$Key]
        ...
        [void]$h.Append("<td style=`"padding:10px 8px;text-align:right;font-weight:bold;color:$dClr;width:55px;`"><font color=`"$dClr`"><b>$DStr</b></font></td>")
        ...
        [void]$h.Append("<td style=`"padding:10px 12px;`">")
        if ($DLevel) {
            [void]$h.Append("<table cellpadding=`"0`" cellspacing=`"0`" border=`"0`"><tr><td style=`"background-color:$dBg;color:$dClr;padding:2px 10px;font-size:10px;font-weight:bold;letter-spacing:0.5px;`"><font color=`"$dClr`">$($DLevel.ToUpper())</font></td></tr></table>")
        }
        [void]$h.Append('</td></tr>')
        $dimIdx++
    }
    ...
    if ($Composite -ge 0) {
        ...
        [void]$h.Append("<td style=`"padding:10px 8px;text-align:right;font-weight:bold;color:$cClr;border-top:2px solid #0078D4;`"><font color=`"$cClr`"><b>$Composite%</b></font></td>")
        ...
        [void]$h.Append("<td style=`"padding:10px 12px;border-top:2px solid #0078D4;`"><table cellpadding=`"0`" cellspacing=`"0`" border=`"0`"><tr><td style=`"background-color:$cBg;color:$cClr;padding:2px 10px;font-size:10px;font-weight:bold;letter-spacing:0.5px;`"><font color=`"$cClr`">$($(Get-MaturityLevel $Composite).ToUpper())</font></td></tr></table></td>")
        [void]$h.Append('</tr>')
    }
    [void]$h.Append('</table></td></tr>')
    [void]$h.Append($divider)
    ...
    if ($strongChecks.Count -gt 0) {
        [void]$h.Append('<tr><td style="padding:20px 40px 16px 40px;">')
        [void]$h.Append("<p style=`"Margin:0;font-size:11px;font-weight:bold;color:#107C10;letter-spacing:1px;padding-bottom:14px;`">&#10003; &#160;KEY SUCCESSES ($($strongChecks.Count))</p>")
    ...
    if ($critFails.Count -gt 0) {
        [void]$h.Append('<tr><td style="padding:20px 40px 16px 40px;">')
        [void]$h.Append("<p style=`"Margin:0;font-size:11px;font-weight:bold;color:#D13438;letter-spacing:1px;padding-bottom:14px;`">&#9888; &#160;TOP FINDINGS ($($critFails.Count))</p>")
        [void]$h.Append('<table cellpadding="0" cellspacing="0" border="0" width="100%" style="font-size:13px;">')
        foreach ($f in $critFails) {
        ...
        if ($medLowFails.Count -gt 0) {
            $medCount = @($medLowFails | Where-Object { $_.Severity -eq 'Medium' }).Count
            $lowCount = @($medLowFails | Where-Object { $_.Severity -eq 'Low' }).Count
            [void]$h.Append("<p style=`"Margin:0;padding-top:10px;font-size:12px;color:#8A8886;background-color:#F5F5F5;padding:10px 12px;`">&#43; $medCount medium and $lowCount low severity findings not shown</p>")
        }
        [void]$h.Append('</td></tr>')
        [void]$h.Append($divider)
    }
    ...
    if ($quickWins.Count -gt 0) {
        [void]$h.Append('<tr><td style="padding:20px 40px 16px 40px;">')
        [void]$h.Append('<p style="Margin:0;font-size:11px;font-weight:bold;color:#0078D4;letter-spacing:1px;padding-bottom:14px;">&#9889; &#160;QUICK WINS</p>')
        [void]$h.Append('<table cellpadding="0" cellspacing="0" border="0" width="100%">')
        $qi = 0
        foreach ($qw in $quickWins) {
            $qi++
            ...
            [void]$h.Append("<p style=`"Margin:0;font-weight:600;color:#242424;`">$(& $hesc $qw.Name)</p>")
            if ($rem) {
                $remText = if ($rem.Length -gt 140) { $rem.Substring(0,140) + '...' } else { $rem }
                [void]$h.Append("<p style=`"Margin:0;font-size:12px;color:#8A8886;padding-top:4px;`">&#8594; $(& $hesc $remText)</p>")
            }
            [void]$h.Append('</td></tr>')
        }
        [void]$h.Append('</table></td></tr>')
...
<table cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
<td style="font-size:11px;color:#A19F9D;">Generated by Windows 365 Assessor v$Global:AppVersion</td>
...
function Export-HtmlReport {
    ...
    $dlg.FileName = "W365_Assessment_$(& { $n = $Global:Assessment.CustomerName -replace '[^\w]','_'; if ($n) { "${n}_" } })$(Get-Date -Format 'yyyyMMdd_HHmmss').html"
...
function Export-CsvReport {
    ...
    $dlg.FileName = "W365_Assessment_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv"
    $dlg.InitialDirectory = $Global:ReportDir
...
function Export-JsonAssessment {
    ...
    $dlg.FileName = "W365_Assessment_$(Get-Date -Format 'yyyyMMdd_HHmmss').json"
    $dlg.InitialDirectory = $Global:ReportDir
    ...
    if ($dlg.ShowDialog() -eq $true) {
        try {
            ...
            if ($Json.SchemaVersion -and $Json.Inventory) {
                ...
                if ($Json.Discovery) {
                    $lblStatCloudPCs.Text     = "$($Json.Discovery.Inventory.CloudPCs.Count)"
                    $lblStatProvPolicies.Text = "$($Json.Discovery.Inventory.ProvisioningPolicies.Count)"
                    $lblStatUserSettings.Text = "$($Json.Discovery.Inventory.UserSettings.Count)"
                    $lblStatANCs.Text         = "$($Json.Discovery.Inventory.AzureNetworkConnections.Count)"
                    $lblStatImages.Text       = "$($Json.Discovery.Inventory.DeviceImages.Count)"
                }
        ...
        if ($RestoredData.Checks -and $RestoredData.Checks.Count -gt 0) {
            ...
            if ($HasWork -gt 0) {
                ...
                if ($RestoredData.Discovery) {
                    $lblStatCloudPCs.Text     = "$($RestoredData.Discovery.Inventory.CloudPCs.Count)"
                    $lblStatProvPolicies.Text = "$($RestoredData.Discovery.Inventory.ProvisioningPolicies.Count)"
                    $lblStatUserSettings.Text = "$($RestoredData.Discovery.Inventory.UserSettings.Count)"
                    $lblStatANCs.Text         = "$($RestoredData.Discovery.Inventory.AzureNetworkConnections.Count)"
                    $lblStatImages.Text       = "$($RestoredData.Discovery.Inventory.DeviceImages.Count)"
                }
