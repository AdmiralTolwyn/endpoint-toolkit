...
function New-ARMParameterObjects {
    ...
    [Optional] Override the Cosmos DB primary region. When specified, the Cosmos DB account is created in this
    region instead of the paired-region default. Useful when the automatic paired region is undesirable.

    .PARAMETER CosmosDBZoneRedundant
    [Optional] Enable zone redundancy for Cosmos DB locations. (Default: False)
    ...
    param(
        ...
        [Parameter(Mandatory = $false)] [string]$CosmosDBRegion = '',
        [Parameter()] [switch]$CosmosDBZoneRedundant,
    ...
    ## Override Cosmos DB region if specified
    if ($CosmosDBRegion) {
        $CosmosDBPrimaryRegionName = $(Get-AzLocation).Where({ $_.Location -eq $CosmosDBRegion }).DisplayName
        if (!$CosmosDBPrimaryRegionName) {
            Write-Error "Invalid CosmosDBRegion: '$CosmosDBRegion'. Use a valid Azure region name (e.g. 'westeurope')."
            return $null
        }
        Write-Verbose -Message "Cosmos DB region overridden to: $CosmosDBPrimaryRegionName"
        $SecondaryRegionName = $null
    } else {
        $CosmosDBPrimaryRegionName = $PrimaryRegionName
    }

    $ZoneRedundant = [bool]$CosmosDBZoneRedundant
    ...
    switch ($ImplementationPerformance) {
        'Developer' {
            ...
            $CosmosDBALocations = @(
                @{
                    locationName     = $CosmosDBPrimaryRegionName
                    failoverPriority = 0
